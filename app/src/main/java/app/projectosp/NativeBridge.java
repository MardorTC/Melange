...
public final class NativeBridge {
  ...
  public String read() {
    try {
      ...
      return "ERROR:No se pudo leer la base local. " + e.getMessage();
    }
  }
  ...
  public String write(String json) {
    try {
      ...
      return "ERROR:No se guardó el cambio: " + e.getMessage();
    }
  }

  @JavascriptInterface
  public void writeAsync(String json, int requestId) {
    new Thread(
            () -> {
              String result = write(json);
              activity.evaluate(
                  "window.nativeSaved(" + requestId + "," + JSONObject.quote(result) + ")");
            },
            "ledger-write")
        .start();
  }
  ...
  public void exportBackup(String json) {
    activity.runOnUiThread(() -> backups.exportBackup(json));
  }

  @JavascriptInterface
  public void importBackup() {
    activity.runOnUiThread(backups::importBackup);
  }

  @JavascriptInterface
  public void installUpdate() {
    activity.runOnUiThread(backups::pickApk);
  }
  ...
  public void configureReminders(String json) {
    try {
      new JSONObject(json);
      activity.getSharedPreferences("reminders", 0).edit().putString("config", json).apply();
      activity.getSystemService(NotificationManager.class).cancel(702);
      ReminderReceiver.schedule(activity);
    } catch (Exception e) {
      activity.message("No se pudieron programar los recordatorios.");
    }
  }
        ...
        () -> {
          if (Build.VERSION.SDK_INT >= 33
            ...
            activity.requestPermissions(
                new String[] {"android.permission.POST_NOTIFICATIONS"}, MainActivity.NOTIFICATIONS);
          else if (!activity
          ...
          } else activity.message("Los avisos del sistema están habilitados.");
        });
  }
  ...
  public String notificationStatus() {
    return activity.getSystemService(NotificationManager.class).areNotificationsEnabled()
        ...
        : "Falta habilitar las notificaciones en Android.";
  }
}
